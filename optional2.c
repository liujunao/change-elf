 ...
 * 我的测试 elf 文件的入口地址为 0x400430
 */
char code[] = {
	0x50, 0x53, 0x51, 0x52,					  //pushq %rax pushq %rbx pushq %rcx pushq %rdx
	0x48, 0xc7, 0xc0, 0x05, 0x00, 0x00, 0x00, //movq $5, %rax
	0x48, 0xc7, 0xc3, 0x86, 0x04, 0x40, 0x00, //movq $filename, %rbx,要将此处的地址换成 elf 的（入口地址 + 0x0047 + 0x000f == 0x400430 + 0x0047 + 0x000f）
	...
	0x48, 0xc7, 0xc1, 0x77, 0x04, 0x40, 0x00, //movq $msg, %rcx，要将此处的地址换成 elf 的（入口地址 + 0x0047 == 0x400430 + 0x0047 ）
	0x48, 0xc7, 0xc2, 0x0d, 0x00, 0x00, 0x00, //movq $len, %rdx
	0xcd, 0x80,								  //int $0x80
	0x5a, 0x59, 0x5b, 0x58,					  //popq %rdx popq %rcx popq %rbx popq %rax

	0xbd, 0x30, 0x04, 0x40, 0x00, 0xff, 0xe5, //跳转指令，跳转到原始 elf 的入口地址
...
int main()
{
	 ...
	 * 写入嵌入的代码
	 */
	fseek(fp, p_header.p_filesz, SEEK_SET);
	 ...
	 * 修改程序头
	 */
	p_header.p_filesz += size; //修改该段的节区总长度，加上嵌入代码长度
